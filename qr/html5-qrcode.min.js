GridSampler = {
    ...
    sampleGrid3: function(d, a, c) {
        for (var b = new BitMatrix(a), e = Array(a << 1), g = 0; g < a; g++) {
            for (var f = e.length, h = g + 0.5, j = 0; j < f; j += 2)
                e[j] = (j >> 1) + 0.5,
                e[j + 1] = h;
...
function ECBlocks(d, a, c) {
    ...
    this.__defineGetter__("NumBlocks", function() {
        for (var b = 0, e = 0; e < this.ecBlocks.length; e++)
            b += this.ecBlocks[e].length;
        return b
    });
...
function Version(d, a, c, b, e, g) {
    ...
    this.buildFunctionPattern = function() {
        ...
        for (var c = this.alignmentPatternCenters.length, a = 0; a < c; a++)
            for (var g = this.alignmentPatternCenters[a] - 2, d = 0; d < c; d++)
                0 == a && (0 == d || d == c - 1) || a == c - 1 && 0 == d || b.setRegion(this.alignmentPatternCenters[d] - 2, g, 5, 5);
        b.setRegion(6, 9, 1, e - 17);
        b.setRegion(9, 6, e - 17, 1);
        6 < this.versionNumber && (b.setRegion(e - 11, 0, 3, 6),
        b.setRegion(0, e - 11, 6, 3));
        return b
    }
    ;
...
Version.decodeVersionInformation = function(d) {
    for (var a = 4294967295, c = 0, b = 0; b < Version.VERSION_DECODE_INFO.length; b++) {
        ...
        if (e == d)
            return this.getVersionForNumber(b + 7);
...
function PerspectiveTransform(d, a, c, b, e, g, f, h, j) {
    ...
    this.times = function(b) {
        return new PerspectiveTransform(this.a11 * b.a11 + this.a21 * b.a12 + this.a31 * b.a13,this.a11 * b.a21 + this.a21 * b.a22 + this.a31 * b.a23,this.a11 * b.a31 + this.a21 * b.a32 + this.a31 * b.a33,this.a12 * b.a11 + this.a22 * b.a12 + this.a32 * b.a13,this.a12 * b.a21 + this.a22 * b.a22 + this.a32 * b.a23,this.a12 * b.a31 + this.a22 * b.a32 + this.a32 * b.a33,this.a13 * b.a11 + this.a23 * b.a12 + this.a33 * b.a13,this.a13 * b.a21 + this.a23 * b.a22 + this.a33 * b.a23,this.a13 * b.a31 + this.a23 * b.a32 + this.a33 * b.a33)
    }
}
...
PerspectiveTransform.squareToQuadrilateral = function(d, a, c, b, e, g, f, h) {
    ...
    dy3 = a - b + g - h;
    if (0 == dy2 && 0 == dy3)
        return new PerspectiveTransform(c - d,e - c,d,b - a,g - b,a,0,0,1);
    dx1 = c - e;
    dx2 = f - e;
    dx3 = d - c + e - f;
    dy1 = b - g;
...
function Detector(d) {
    ...
    ;
    this.sizeOfBlackWhiteBlackRunBothWays = function(a, c, b, e) {
        var g = this.sizeOfBlackWhiteBlackRun(a, c, b, e)
          , d = 1;
        b = a - (b - a);
        0 > b ? (d = a / (a - b),
        b = 0) : b >= qrcode.width && (d = (qrcode.width - 1 - a) / (b - a),
        b = qrcode.width - 1);
        e = Math.floor(c - (e - c) * d);
        d = 1;
        0 > e ? (d = c / (c - e),
        e = 0) : e >= qrcode.height && (d = (qrcode.height - 1 - c) / (e - c),
        e = qrcode.height - 1);
    ...
    this.createTransform = function(a, c, b, e, d) {
        ...
        null != e ? (f = e.X,
        e = e.Y,
        h = j = d - 3) : (f = c.X - a.X + b.X,
        e = c.Y - a.Y + b.Y,
        h = j = d);
        return PerspectiveTransform.quadrilateralToQuadrilateral(3.5, 3.5, d, 3.5, h, j, 3.5, d, a.X, a.Y, c.X, c.Y, f, e, b.X, b.Y)
    }
    ;
    ...
    this.processFinderPatternInfo = function(a) {
        ...
        if (0 < f.AlignmentPatternCenters.length) {
            ...
            for (h = Math.floor(c.Y + h * (b.Y - c.Y + a.Y - c.Y)); ; ) {
...
FormatInformation.doDecodeFormatInformation = function(d) {
    for (var a = 4294967295, c = 0, b = 0; b < FORMAT_INFO_DECODE_LOOKUP.length; b++) {
        var e = FORMAT_INFO_DECODE_LOOKUP[b]
          , g = e[0];
        if (g == d)
            return new FormatInformation(e[1]);
...
function BitMatrix(d, a) {
    ...
    ;
    this.clear = function() {
        for (var b = this.bits.length, e = 0; e < b; e++)
            this.bits[e] = 0
    }
    ;
    this.setRegion = function(b, e, c, a) {
        ...
        c = b + c;
        a = e + a;
        if (a > this.height || c > this.width)
            throw "The region must fit inside the matrix";
        for (; e < a; e++)
            for (var d = e * this.rowSize, j = b; j < c; j++)
                this.bits[d + (j >> 5)] |= 1 << (j & 31)
    }
}
...
DataBlock.getDataBlocks = function(d, a, c) {
    ...
    for (e = c.length - 1; 0 <= e && c[e].codewords.length != a; )
        e--;
    e++;
...
function BitMatrixParser(d) {
    ...
    this.readVersion = function() {
        ...
        var c = this.bitMatrix.Dimension
          , b = c - 17 >> 2;
        if (6 >= b)
            return Version.getVersionForNumber(b);
        for (var b = 0, e = c - 11, a = 5; 0 <= a; a--)
            for (var d = c - 9; d >= e; d--)
                b = this.copyBit(d, a, b);
        this.parsedVersion = Version.decodeVersionInformation(b);
        if (null != this.parsedVersion && this.parsedVersion.DimensionForVersion == c)
            return this.parsedVersion;
        b = 0;
        for (d = 5; 0 <= d; d--)
            for (a = c - 9; a >= e; a--)
    ...
    this.readCodewords = function() {
        ...
        for (var e = b.buildFunctionPattern(), a = !0, d = Array(b.TotalCodewords), h = 0, j = 0, k = 0, n = c - 1; 0 < n; n -= 2) {
            6 == n && n--;
            for (var l = 0; l < c; l++)
                for (var s = a ? c - 1 - l : l, r = 0; 2 > r; r++)
                    e.get_Renamed(n - r, s) || (k++,
                    j <<= 1,
                    this.bitMatrix.get_Renamed(n - r, s) && (j |= 1),
                    8 == k && (d[h++] = j,
                    j = k = 0));
            a ^= 1
        }
        if (h != b.TotalCodewords)
...
function ReedSolomonDecoder(d) {
    ...
    this.decode = function(a, c) {
        for (var b = new GF256Poly(this.field,a), e = Array(c), d = 0; d < e.length; d++)
            e[d] = 0;
        ...
        for (var e = this.field.One, d = this.field.Zero, f = this.field.Zero, h = this.field.One; c.Degree >= Math.floor(b / 2); ) {
            ...
            for (d = this.field.inverse(d); c.Degree >= a.Degree && !c.Zero; ) {
                var j = c.Degree - a.Degree
                  ...
                  , h = h.addOrSubtract(this.field.buildMonomial(j, l));
    ...
    this.findErrorLocations = function(a) {
        var c = a.Degree;
        if (1 == c)
            return Array(a.getCoefficient(1));
        for (var b = Array(c), e = 0, d = 1; 256 > d && e < c; d++)
            0 == a.evaluateAt(d) && (b[e] = this.field.inverse(d),
            e++);
        if (e != c)
    ...
    this.findErrorMagnitudes = function(a, c, b) {
        for (var e = c.length, d = Array(e), f = 0; f < e; f++) {
            for (var h = this.field.inverse(c[f]), j = 1, k = 0; k < e; k++)
                f != k && (j = this.field.multiply(j, GF256.addOrSubtract(1, this.field.multiply(c[k], h))));
...
function GF256Poly(d, a) {
    ...
    var c = a.length;
    if (1 < c && 0 == a[0]) {
        for (var b = 1; b < c && 0 == a[b]; )
            b++;
        if (b == c)
            this.coefficients = d.Zero.coefficients;
        else {
            this.coefficients = Array(c - b);
            for (c = 0; c < this.coefficients.length; c++)
                this.coefficients[c] = 0;
            for (c = 0; c < this.coefficients.length; c++)
                this.coefficients[c] = a[b + c]
        }
    } else
        this.coefficients = a;
    ...
    this.evaluateAt = function(b) {
        ...
        if (1 == b) {
            for (var a = b = 0; a < c; a++)
                b = GF256.addOrSubtract(b, this.coefficients[a]);
            return b
        }
        for (var d = this.coefficients[0], a = 1; a < c; a++)
            d = GF256.addOrSubtract(this.field.multiply(b, d), this.coefficients[a]);
        return d
    }
    ;
    this.addOrSubtract = function(b) {
        ...
        for (var c = Array(b.length), h = b.length - a.length, j = 0; j < h; j++)
            c[j] = b[j];
        for (j = h; j < b.length; j++)
            c[j] = GF256.addOrSubtract(a[j - h], b[j]);
    ...
    this.multiply1 = function(b) {
        ...
        for (var d = b.length, j = Array(c + d - 1), k = 0; k < c; k++)
            for (var n = a[k], l = 0; l < d; l++)
                j[k + l] = GF256.addOrSubtract(j[k + l], this.field.multiply(n, b[l]));
    ...
    this.multiply2 = function(b) {
        ...
        if (1 == b)
            return this;
        for (var a = this.coefficients.length, c = Array(a), d = 0; d < a; d++)
            c[d] = this.field.multiply(this.coefficients[d], b);
    ...
    this.multiplyByMonomial = function(b, a) {
        ...
        if (0 == a)
            return this.field.Zero;
        for (var c = this.coefficients.length, d = Array(c + b), j = 0; j < d.length; j++)
            d[j] = 0;
        for (j = 0; j < c; j++)
            d[j] = this.field.multiply(this.coefficients[j], a);
    ...
    this.divide = function(b) {
        ...
        if (b.Zero)
            throw "Divide by 0";
        for (var a = this.field.Zero, c = this, d = b.getCoefficient(b.Degree), d = this.field.inverse(d); c.Degree >= b.Degree && !c.Zero; )
            var j = c.Degree - b.Degree
              , k = this.field.multiply(c.getCoefficient(c.Degree), d)
...
function GF256(d) {
    ...
    this.buildMonomial = function(b, c) {
        ...
        if (0 == c)
            return zero;
        for (var a = Array(b + 1), d = 0; d < a.length; d++)
            a[d] = 0;
        a[0] = c;
...
Decoder.correctErrors = function(d, a) {
    for (var c = d.length, b = Array(c), e = 0; e < c; e++)
        b[e] = d[e] & 255;
    c = d.length - a;
    try {
...
Decoder.decode = function(d) {
    ...
    d = a.readVersion();
    for (var c = a.readFormatInformation().ErrorCorrectionLevel, a = a.readCodewords(), a = DataBlock.getDataBlocks(a, d, c), b = 0, e = 0; e < a.Length; e++)
        b += a[e].NumDataCodewords;
...
qrcode = {
    ...
    decode: function(d) {
        ...
        b.onload = function() {
            ...
            null != d && (d = d.getContext("2d"),
            d.clearRect(0, 0, 320, 240),
            d.drawImage(b, 0, 0, 320, 240));
            a.width = b.width;
            a.height = b.height;
            c.drawImage(b, 0, 0);
        ...
        if (qrcode.debug) {
            for (var b = 0; b < qrcode.height; b++)
                for (var e = 0; e < qrcode.width; e++) {
                    ...
                    qrcode.imagedata.data[g + 2] = c[e + b * qrcode.width] ? 255 : 0
                }
    ...
    getMiddleBrightnessPerArea: function(d) {
        ...
        for (e = 0; 4 > e; e++)
            for (g = 0; 4 > g; g++)
                d[g][e] = Math.floor((b[g][e][0] + b[g][e][1]) / 2);
        return d
    },
    grayScaleToBitmap: function(d) {
        for (var a = qrcode.getMiddleBrightnessPerArea(d), c = a.length, b = Math.floor(qrcode.width / c), e = Math.floor(qrcode.height / c), g = Array(qrcode.height * qrcode.width), f = 0; f < c; f++)
            for (var h = 0; h < c; h++)
                for (var j = 0; j < e; j++)
                    for (var k = 0; k < b; k++)
                        g[b * h + k + (e * f + j) * qrcode.width] = d[b * h + k + (e * f + j) * qrcode.width] < a[h][f] ? !0 : !1;
        return g
    },
    grayscale: function() {
...
function FinderPatternFinder() {
    ...
    this.crossCheckVertical = function(d, a, c, b) {
        for (var e = this.image, g = qrcode.height, f = this.CrossCheckStateCount, h = d; 0 <= h && e[a + h * qrcode.width]; )
            f[2]++,
            h--;
        if (0 > h)
            return NaN;
        for (; 0 <= h && !e[a + h * qrcode.width] && f[1] <= c; )
            f[1]++,
            h--;
        if (0 > h || f[1] > c)
            return NaN;
        for (; 0 <= h && e[a + h * qrcode.width] && f[0] <= c; )
            f[0]++,
            h--;
        if (f[0] > c)
            return NaN;
        for (h = d + 1; h < g && e[a + h * qrcode.width]; )
            f[2]++,
            h++;
        if (h == g)
            return NaN;
        for (; h < g && !e[a + h * qrcode.width] && f[3] < c; )
            f[3]++,
            h++;
        if (h == g || f[3] >= c)
            return NaN;
        for (; h < g && e[a + h * qrcode.width] && f[4] < c; )
            f[4]++,
            h++;
        return f[4] >= c || 5 * Math.abs(f[0] + f[1] + f[2] + f[3] + f[4] - b) >= 2 * b ? NaN : this.foundPatternCross(f) ? this.centerFromEnd(f, h) : NaN
    }
    ;
    this.crossCheckHorizontal = function(d, a, c, b) {
        for (var e = this.image, g = qrcode.width, f = this.CrossCheckStateCount, h = d; 0 <= h && e[h + a * qrcode.width]; )
            f[2]++,
            h--;
        if (0 > h)
            return NaN;
        for (; 0 <= h && !e[h + a * qrcode.width] && f[1] <= c; )
            f[1]++,
            h--;
        if (0 > h || f[1] > c)
            return NaN;
        for (; 0 <= h && e[h + a * qrcode.width] && f[0] <= c; )
            f[0]++,
            h--;
        if (f[0] > c)
            return NaN;
        for (h = d + 1; h < g && e[h + a * qrcode.width]; )
            f[2]++,
            h++;
        if (h == g)
            return NaN;
        for (; h < g && !e[h + a * qrcode.width] && f[3] < c; )
            f[3]++,
            h++;
        if (h == g || f[3] >= c)
            return NaN;
        for (; h < g && e[h + a * qrcode.width] && f[4] < c; )
            f[4]++,
            h++;
        return f[4] >= c || 5 * Math.abs(f[0] + f[1] + f[2] + f[3] + f[4] - b) >= b ? NaN : this.foundPatternCross(f) ? this.centerFromEnd(f, h) : NaN
    }
    ;
    this.handlePossibleCenter = function(d, a, c) {
        ...
        if (!isNaN(a) && (c = this.crossCheckHorizontal(Math.floor(c), Math.floor(a), d[2], b),
        !isNaN(c))) {
            d = b / 7;
    ...
    this.selectBestPatterns = function() {
        ...
        if (3 < d) {
            for (var a = 0, c = 0; c < d; c++)
                a += this.possibleCenters[c].EstimatedModuleSize;
            d = a / d;
            for (c = 0; c < this.possibleCenters.length && 3 < this.possibleCenters.length; c++)
                Math.abs(this.possibleCenters[c].EstimatedModuleSize - d) > 0.2 * d && (this.possibleCenters.remove(c),
                c--)
        }
        ...
        for (var a = null, c = 0; c < d; c++) {
            ...
            if (b.Count >= CENTER_QUORUM)
                if (null == a)
                    ...
                    Math.floor((Math.abs(a.X - b.X) - Math.abs(a.Y - b.Y)) / 2)
        }
        return 0
    }
    ;
    this.haveMultiplyConfirmedCenters = function() {
        for (var d = 0, a = 0, c = this.possibleCenters.length, b = 0; b < c; b++) {
            ...
            e.Count >= CENTER_QUORUM && (d++,
            a += e.EstimatedModuleSize)
        }
        if (3 > d)
            return !1;
        for (var d = a / c, g = 0, b = 0; b < c; b++)
            e = this.possibleCenters[b],
            g += Math.abs(e.EstimatedModuleSize - d);
        return g <= 0.05 * a
    }
    ;
    this.findFinderPattern = function(d) {
        ...
        for (var e = !1, g = Array(5), f = b - 1; f < a && !e; f += b) {
            ...
            for (var h = g[4] = 0, j = 0; j < c; j++)
                if (d[j + f * qrcode.width])
                    1 == (h & 1) && h++,
                    g[h]++;
                else if (0 == (h & 1))
                    if (4 == h)
                        if (this.foundPatternCross(g)) {
                            if (h = this.handlePossibleCenter(g, f, j))
                                b = 2,
                                this.hasSkipped ? e = this.haveMultiplyConfirmedCenters() : (h = this.findRowSkip(),
                                h > g[2] && (f += h - g[2] - b,
                                j = c - 1));
                            else {
                                do
                                    j++;
                                while (j < c && !d[j + f * qrcode.width]);j--
                            }
                            h = 0;
                            g[0] = 0;
                            g[1] = 0;
                    ...
                    g[h]++;
            this.foundPatternCross(g) && this.handlePossibleCenter(g, f, c) && (b = g[0],
            this.hasSkipped && (e = haveMultiplyConfirmedCenters()))
        }
...
function AlignmentPatternFinder(d, a, c, b, e, g, f) {
    ...
    this.crossCheckVertical = function(b, a, c, d) {
        ...
        f[1] = 0;
        f[2] = 0;
        for (var m = b; 0 <= m && e[a + m * qrcode.width] && f[1] <= c; )
            f[1]++,
            m--;
        if (0 > m || f[1] > c)
            return NaN;
        for (; 0 <= m && !e[a + m * qrcode.width] && f[0] <= c; )
            f[0]++,
            m--;
        if (f[0] > c)
            return NaN;
        for (m = b + 1; m < g && e[a + m * qrcode.width] && f[1] <= c; )
            f[1]++,
            m++;
        if (m == g || f[1] > c)
            return NaN;
        for (; m < g && !e[a + m * qrcode.width] && f[2] <= c; )
            f[2]++,
            m++;
    ...
    this.handlePossibleCenter = function(b, a, c) {
        var d = b[0] + b[1] + b[2];
        c = this.centerFromEnd(b, c);
        a = this.crossCheckVertical(a, Math.floor(c), 2 * b[1], d);
        if (!isNaN(a)) {
            b = (b[0] + b[1] + b[2]) / 3;
            for (var d = this.possibleCenters.length, e = 0; e < d; e++)
                if (this.possibleCenters[e].aboutEquals(b, a, c))
                    return new AlignmentPattern(c,a,b);
    ...
    this.find = function() {
        for (var a = this.startX, e = this.height, g = a + b, f = c + (e >> 1), l = [0, 0, 0], s = 0; s < e; s++) {
            var r = f + (0 == (s & 1) ? s + 1 >> 1 : -(s + 1 >> 1));
            l[0] = 0;
            l[1] = 0;
            l[2] = 0;
            for (var m = a; m < g && !d[m + qrcode.width * r]; )
                m++;
            for (var q = 0; m < g; ) {
                if (d[m + r * qrcode.width])
                    if (1 == q)
                        ...
                        if (this.foundPatternCross(l) && (q = this.handlePossibleCenter(l, r, m),
                        null != q))
                            return q;
                        l[0] = l[2];
                        l[1] = 1;
            ...
            if (this.foundPatternCross(l) && (q = this.handlePossibleCenter(l, r, g),
            null != q))
                return q
        }
...
function QRCodeDataBlockReader(d, a, c) {
    ...
    this.getNextBits = function(b) {
        ...
        if (b < this.bitPointer + 1 + 8) {
            for (var d = 0, a = 0; a < this.bitPointer + 1; a++)
                d += 1 << a;
    ...
    this.getFigureString = function(b) {
        var a = 0
          , c = "";
        do
            3 <= b ? (a = this.getNextBits(10),
            100 > a && (c += "0"),
            10 > a && (c += "0"),
            b -= 3) : 2 == b ? (a = this.getNextBits(7),
            10 > a && (c += "0"),
            b -= 2) : 1 == b && (a = this.getNextBits(4),
            b -= 1),
            c += a;
        while (0 < b);return c
    }
    ...
    this.__defineGetter__("DataByte", function() {
        ...
        do {
            ...
            if (1 != a && 2 != a && 4 != a && 8 != a)
                throw "Invalid mode: " + a + " in (block:" + this.blockPointer + " bit:" + this.bitPointer + ")";
            dataLength = this.getDataLength(a);
            if (1 > dataLength)
                throw "Invalid data length: " + dataLength;
            switch (a) {
            case 1:
                for (var a = this.getFigureString(dataLength), c = Array(a.length), d = 0; d < a.length; d++)
                    c[d] = a.charCodeAt(d);
                b.push(c);
                break;
            case 2:
            ...
            case 4:
                ...
                break;
            case 8:
                a = this.getKanjiString(dataLength),
                b.push(a)
            }
        } while (1);return b
    })
...
;(function(d) {
    d.fn.html5_qrcode = function(a, c, b,DDD) {
        var e = this.height()
          , g = this.width();
        null == e && (e = 250);
        null == g && (g = 300);
        ...
        navigator.getUserMedia ? navigator.getUserMedia({
            video: {deviceId: DDD}
        }, e, b) : console.log("Native web camera streaming (getUserMedia) not supported in this browser.");
        qrcode.callback = a
    }
})(jQuery);
